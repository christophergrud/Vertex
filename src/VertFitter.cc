...
// $Id: VertFitter.cc,v 1.56 2011/12/23 08:13:39 innocent Exp $
//
...
VertFitter::VertFitter(const edm::ParameterSet& theParameters,
  ...
  //  -whether to reconstruct MyVertices
  ...
  myMass = theParameters.getParameter<double>(string("myMass"));
  myMassCut = theParameters.getParameter<double>(string("myMassCut"));
  ...
  vtxFitter = theParameters.getParameter<edm::InputTag>("vertexFitter");  // *************************** VertFitter --> vertexFitter **********
...
VertFitter::~VertFitter() {
}

// Method containing the algorithm for vertex reconstruction
void VertFitter::fitAll(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  /* for(unsigned int indx = 0; indx < theTrackHandle->size(); indx++) {
    ...
    if( tmpRef->normalizedChi2() < tkChi2Cut && tmpRef->numberOfValidHits() >= tkNhitsCut ) {
  ...
  int howManyTrackerMuons = 0;

  for (std::vector<reco::Muon>::const_iterator it = theMuonHandle->begin(); it != theMuonHandle->end(); ++it) {
    if (!it->isTrackerMuon()) continue;
    howManyTrackerMuons++;
    TrackRef tmpRef = it->innerTrack();
    bool quality_ok = true;
    if (qualities.size()!=0) {
      quality_ok = false;
      for (unsigned int ndx_ = 0; ndx_ < qualities.size(); ndx_++) {
  ...
  //std::cout<<" "<<std::endl;
  //std::cout << "Number of Reco Muons = " << theMuonHandle->size() << "  ||  Number of stored Tracks = " << theTrackRefs.size() << std::endl;
  //std::cout << "There are " << howManyTrackerMuons << " tracker muons. " << std::endl;
  //std::cout<<" "<<std::endl;
  ...
  for(unsigned int trdx1 = 0; trdx1 < theTrackRefs.size(); trdx1++) {
    ...
    for(unsigned int trdx2 = trdx1 + 1; trdx2 < theTrackRefs.size(); trdx2++) {
      ...
      //  and references to be sent to the KalmanVertFitter
      ...
      //std::cout<<"Vertex ndof = " << theVtx.ndof() << std::endl;
      ...
      GlobalPoint beamSpotPos(theBeamSpotHandle->position().x(), theBeamSpotHandle->position().y(), theBeamSpotHandle->position().z());

      SMatrixSym3D totalCov = theBeamSpotHandle->rotatedCovariance3D() + theVtx.covariance();
      SVector3 distanceVector(vtxPos.x() - beamSpotPos.x(), vtxPos.y() - beamSpotPos.y(), 0.); //so that we get radial values only, since z beamSpot uncertainty is huge
      ...
      double pMuonE = sqrt( positiveP.mag2() + muonMassSquared ); // ***************************************************************************
      double nMuonE = sqrt( negativeP.mag2() + muonMassSquared ); // ***************************************************************************
      double kShortETot = piPlusE + piMinusE;
      double lambdaEtot = protonE + piMinusE;
      double lambdaBarEtot = antiProtonE + piPlusE;
      double myVertexETot = pMuonE + nMuonE;  // ***********************************************************************************************
      ...
      const Particle::LorentzVector myVertexP4(totalP.x(), totalP.y(), totalP.z(), myVertexETot);  // *****************************************
      ...
      VertexCompositeCandidate* theMyVertex = 0;  // *******************************************************************************************
      ...
      if (storeMyVertices) theMyVertex = new VertexCompositeCandidate(0, myVertexP4, vtx, vtxCov, vtxChi2, vtxNdof);  // ***********************

      // Create daughter candidates for the VertexCompositeCandidates
      RecoChargedCandidate thePiPlusCand(1, Particle::LorentzVector(positiveP.x(), positiveP.y(), positiveP.z(), piPlusE), vtx);
      thePiPlusCand.setTrack(positiveTrackRef);
      
      RecoChargedCandidate thePiMinusCand(-1, Particle::LorentzVector(negativeP.x(), negativeP.y(), negativeP.z(), piMinusE), vtx);
      thePiMinusCand.setTrack(negativeTrackRef);
      
 
      RecoChargedCandidate theProtonCand(1, Particle::LorentzVector(positiveP.x(), positiveP.y(), positiveP.z(), protonE), vtx);
      theProtonCand.setTrack(positiveTrackRef);

      RecoChargedCandidate theAntiProtonCand(-1, Particle::LorentzVector(negativeP.x(), negativeP.y(), negativeP.z(), antiProtonE), vtx);
      theAntiProtonCand.setTrack(negativeTrackRef);

      RecoChargedCandidate thePMuonCand(1, Particle::LorentzVector(positiveP.x(), positiveP.y(), positiveP.z(), pMuonE), vtx);  // ***************
      thePMuonCand.setTrack(positiveTrackRef); // ************************************************************************************************

      RecoChargedCandidate theNMuonCand(-1, Particle::LorentzVector(negativeP.x(), negativeP.y(), negativeP.z(), nMuonE), vtx); // ***************
      theNMuonCand.setTrack(negativeTrackRef); // ************************************************************************************************
      ...
      if( doKshorts ) {
	      ...
	      addp4.set( *theKshort );
	      if( theKshort->mass() < kShortMass + kShortMassCut && theKshort->mass() > kShortMass - kShortMassCut ) {
      ...
      if( doLambdas && theLambda ) {
	      ...
	      addp4.set( *theLambda );
	      if( theLambda->mass() < lambdaMass + lambdaMassCut && theLambda->mass() > lambdaMass - lambdaMassCut ) {
	      ...
	      if( theLambdaBar->mass() < lambdaMass + lambdaMassCut && theLambdaBar->mass() > lambdaMass - lambdaMassCut ) {
	        theLambdas.push_back( *theLambdaBar );
	      }
      }

      if (storeMyVertices) {
        theMyVertex->addDaughter(thePMuonCand);
        theMyVertex->addDaughter(theNMuonCand);
        theMyVertex->setPdgId(6001113);
        addp4.set( *theMyVertex );
        if (theMyVertex->mass() < myMass + myMassCut && theMyVertex->mass() > myMass - myMassCut){
          theMyVertices.push_back( *theMyVertex);
        }
      }
      ...
      if(theMyVertex) delete theMyVertex;
      theKshort = theLambda = theLambdaBar = theMyVertex = 0;
...
// Get methods
const reco::VertexCompositeCandidateCollection& VertFitter::getKshorts() const {
  return theKshorts;
}

const reco::VertexCompositeCandidateCollection& VertFitter::getLambdas() const {
  return theLambdas;
}

const reco::VertexCompositeCandidateCollection& VertFitter::getMyVertices() const {
  return theMyVertices;
}

// Experimental
double VertFitter::findV0MassError(const GlobalPoint &vtxPos, std::vector<reco::TransientTrack> dauTracks) { 
  return -1.;
}
